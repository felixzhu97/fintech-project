---
name: developer
description: Feature development for FinPulse — XP, DDD, BDD, TDD, Glossary naming, Apple HIG minimal UX, living docs sync (C4 / domain / product), and mandatory commit/PR standards (why body + References from official docs and research). Use when implementing features, writing tests, committing, opening PRs, UI work, or DDD/TDD/BDD/XP/clean-code tasks.
---
...
1. Layers: [architecture](../../rules/architecture.mdc) — `handler/web → application → domain ← infrastructure`
2. No `domain/port`, `adapter/in|out`, `*Port` in new code
3. Tests: `should expected result when condition` (spaces; Go: `TestType_Behavior`)
4. Names: domain Preferred Term ([docs/en/rd/domain/](../../../docs/en/rd/domain/)) + [clean-code-naming](references/clean-code-naming.md)
5. UI: Apple HIG + Emotion + [apple-minimal-ux](references/apple-minimal-ux.md)
...
8. **Living docs**: when the change hits the trigger matrix, update domain / C4 / product docs in the **same PR** — see §4 and [living-docs](references/living-docs.md)
...
| | Rule |
|--|------|
| Name | `should expected result when condition` (Go: `TestType_Behavior`) |
...
| Concept | Location (Go) |
|---------|----------------|
| Entity / Aggregate | `internal/domain/` — rich behavior where rules belong |
| Value Object | `internal/domain/` — immutable fields / typed IDs |
| Repository | application ports → impl in `infrastructure/persistence/` |
| Use Case / Service | `internal/application/` — orchestration only |
| HTTP | `internal/handler/` — no business rules |

Detail: [ddd-rich-model](references/ddd-rich-model.md)

### 3. Naming

Glossary Preferred Term first → Clean Code form. No synonyms (`Holding` vs `Position`). New concept → update domain docs in the same change.
...
| Document | Path |
|----------|------|
| Domain | [docs/en/rd/domain/](../../../docs/en/rd/domain/) · [docs/zh/rd/domain/](../../../docs/zh/rd/domain/) |
| C4 | [docs/en/rd/c4/](../../../docs/en/rd/c4/) · [docs/zh/rd/c4/](../../../docs/zh/rd/c4/) |
| Product | [docs/en/product/](../../../docs/en/product/) · [docs/zh/product/](../../../docs/zh/product/) |

| Change | Must update |
|--------|-------------|
| New/changed Preferred Term, module package, route, API prefix, business concept | Domain docs |
...
| New user-visible capability, delivery status, primary nav add/remove | Product docs |
...
| Domain docs | [docs/en/rd/domain/](../../../docs/en/rd/domain/) |
| C4 model | [docs/en/rd/c4/](../../../docs/en/rd/c4/) |
| Product docs | [docs/en/product/](../../../docs/en/product/) |
